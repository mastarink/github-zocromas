...
/* static int duf_digest_dirent_content2( duf_depthinfo_t * H_PDI, duf_sccb_handle_t * sccbh MAS_UNUSED ); */
static DR( MOD, digest_dirent_content2, duf_depthinfo_t * pdi_unused, duf_sccb_handle_t * sccbh );
...
/* " INDEXED BY " DUF_SQL_TABLES_SD5 " _uniq WHERE " MOD_DIGEST_DATA_S " sum1 =: digestSum1 AND " MOD_DIGEST_DATA_S " sum2 =: digestSum2 AND " MOD_DIGEST_DATA_S " sum3 =:digestSum3 */
  ...
  DUF_SQL_SE_START_STMT( H_PDI, select_sha1, sql, pstmt_local );
     ...
     unsigned long long *digest64, const char *msg MAS_UNUSED, int need_id )
{
  ...
  if ( digest64 && digest64[2] && digest64[1] && digest64[0] )
  {
    if ( !duf_get_config_flag_disable_insert(  ) )
    {
      ...
      MAST_TRACE( digest, 0, "%08llx%016llx%016llx %s%s", digest64[2], digest64[1], digest64[0], duf_levinfo_path( H_PDI ), msg );
      DUF_SQL_SE_START_STMT( H_PDI, insert_sha1, sql, pstmt_local );
      ...
      DUF_SQL_SE_END_STMT( H_PDI, insert_sha1, pstmt_local );        /* clears SQL_ROW / SQL_DONE */
    }
    ...
    if ( ( QISERR1_N( SQL_CONSTRAINT ) || QNOERR ) && !changes )
    {
      if ( need_id )
#if 0
        digestid = duf_pdistat2file_digestid_existed( H_PDI, digest64[2], digest64[1], digest64[0], QPERRIND );
#else
        digestid = CRP( pdistat2file_digestid_existed, H_PDI, sccbh, digest64[2], digest64[1], digest64[0] );
#endif
    }
...
SR( MOD, digest_dirent_content2, duf_depthinfo_t * pdi_unused MAS_UNUSED, duf_sccb_handle_t * sccbh MAS_UNUSED )
{
  ...
  if ( QNOERR )
  {
...
#if 0
    digestid = duf_insert_digest_uni( H_PDI, pdgst, fname /* for dbg message only */ , 1 /*need_id */ , QPERRIND );
#else
    ...
    if ( digestid )
    {
      ...
      if ( !duf_get_config_flag_disable_update(  ) )
      {
        ...
        DUF_SQL_SE_START_STMT( H_PDI, update_sha1id, sql, pstmt_local );
        MAST_TRACE( mod, 3, "S:%s", sql );
        DUF_SQL_SE_BIND_LL( digestId, digestid, pstmt_local );
        DUF_SQL_SE_BIND_LL( dataId, filedataid, pstmt_local );
        DUF_SQL_SE_STEPC( pstmt_local );
        DUF_SQL_SE_CHANGES( H_PDI, changes, pstmt_local );
        DUF_SQL_SE_END_STMT( H_PDI, update_sha1id, pstmt_local );    /* clears SQL_ROW / SQL_DONE */
        duf_pdi_reg_changes( H_PDI, changes );
      }
  ...
  ER( MOD, digest_dirent_content2, duf_depthinfo_t * pdi_unused, duf_sccb_handle_t * sccbh );
}
