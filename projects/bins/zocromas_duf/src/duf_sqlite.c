...
int
duf_sqlite_exec_c( const char *sql, int constraint_ignore )
{
  int r3;
  char *emsg = ( char * ) NULL;
...
int
duf_vsqlite_c( const char *fmt, int constraint_ignore, va_list args )
{
  int r3 = 0;
  char *sql;
  ...
  r3 = duf_sqlite_exec_c( sql, constraint_ignore );


  sqlite3_free( sql );
  duf_dbgfunc( DBG_ENDRS, __func__, __LINE__, r3, sql );
  return ( r3 );
}
...
duf_sqlite_vselect( duf_sql_select_cb_t sel_cb, void *sel_cb_udata, duf_scan_callback_file_t str_cb, void *str_cb_udata,
                    duf_depthinfo_t * pdi, duf_scan_callbacks_t * sccb, duf_dirhandle_t * pdhu, const char *fmt, va_list args )
{
  int r3 = 0;
  int row, column;
  ...
  DUF_TRACE( sql, 2, "[%s] r3=%d;  %u rows", sql, r3, row );
 ...
 * */
        ...
        {
          DUF_TRACE( current, 0, "pdhu : %d sel_cb:'%s'", pdhu ? ( pdhu->dfd ? 2 : 1 ) : 0, duf_dbg_funname( ( duf_anyhook_t ) sel_cb ) );
