  ...
  if ( str_cb2 )
  {
...

    {
      ...
      if ( sccbh->rows && sccbh->rows->prev )
      {
        ...
        sha1id0 = duf_sccb_row_get_number( new_row, "sha1id" );
        sha1id1 = duf_sccb_row_get_number( sccbh->rows, "sha1id" );
        eq = ( sha1id0 == sha1id1 );
        {
          if ( !eq )
          {
            QT( "@@--- %lld ? %lld : %p:%p", sha1id0, sha1id1, sccbh->rows, sccbh->rows->prev );
