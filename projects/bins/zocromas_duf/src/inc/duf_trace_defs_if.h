...
/* ###################################################################### */
#  define DUF_IF_TR_WHAT_C(  _cfg, _what, _name )		(( _cfg && !_cfg->opt.trace.nonew) ? _cfg->_what._name: 1 )
#  define DUF_IF_TR_WHAT( _what, _name )			DUF_IF_TR_WHAT_C( DUF_TRACE_CONFIG,	_what,		_name )
#  define DUF_IF_TR( _name )					DUF_IF_TR_WHAT(				opt.trace,	_name)

#  define DUF_IF_TR_WHATN_C( _cfg, _what, _name, _min )		( _cfg && !_cfg->opt.trace.nonew && _cfg->_what._name > _min )
#  define DUF_IF_TR_WHATN( _what, _name, _min )			DUF_IF_TR_WHATN_C( DUF_TRACE_CONFIG,	_what,		_name, _min )
#  define DUF_IF_TRN( _name, _min )				DUF_IF_TR_WHATN( opt.trace, _name, _min)
...
/* #  define DUF_TR_C( _cfg,_name, ... )                     DUF_TR_WHAT_C( _cfg, opt.trace, _name, __VA_ARGS__ ) */
/* #  define DUF_TR(_name, ...)                             DUF_TR_C( DUF_TRACE_CONFIG, __VA_ARGS__ )                */

/* ###################################################################### */

/* #  define DUF_IF_TRACE_WHAT(_what,_name) (DUF_TRACE_CONFIG && !DUF_TRACE_CONFIG->opt.trace.nonew && DUF_TRACE_CONFIG->_what._name ) */

#  define DUF_IF_TRACE_WHAT_C(  _cfg, _what, _name )		(( _cfg && !_cfg->opt.trace.nonew) ? _cfg->_what._name : 1 )

#  define DUF_IF_TRACE_WHAT( _what, _name )			DUF_IF_TRACE_WHAT_C( DUF_TRACE_CONFIG,	_what,		_name )
#  define DUF_IF_TRACE( _name )					DUF_IF_TRACE_WHAT(			opt.trace,	_name )

#  define DUF_IF_TRACE_WHATN_C( _cfg, _what, _name, _min )	( _cfg && !_cfg->opt.trace.nonew && _cfg->_what._name > _min )
#  define DUF_IF_TRACE_WHATN( _what, _name, _min )		DUF_IF_TRACE_WHATN_C( DUF_TRACE_CONFIG,	_what,		_name, _min )
#  define DUF_IF_TRACEN( _name, _min )				DUF_IF_TRACE_WHATN(			opt.trace,	_name, _min )
