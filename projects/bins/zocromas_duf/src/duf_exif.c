...
copy_jpeg_by_date( void )
{
  ...
  r = duf_sql_select( duf_sql_copy_jpeg_by_date, NULL, NULL, 0,
                      "SELECT duf_filedatas.id, duf_filedatas.dev, duf_filedatas.inode, duf_filenames.pathid, duf_exif.datetime, duf_filenames.name "
...
duf_update_exif( void )
{
  ...
  r = duf_sql_select( duf_sql_update_exif, NULL, NULL, 0,
...
/*   {                                                                                                                                                   */
/*     sql = sqlite3_mprintf( "SELECT duf_filedatas.id, duf_filedatas.dev, duf_filedatas.inode, duf_filenames.pathid, duf_filenames.name "               */
/*                            " FROM duf_filedatas " " LEFT JOIN duf_filenames ON (duf_filedatas.id=duf_filenames.dataid) "                              */
/*                            " LEFT JOIN duf_exif ON (duf_exif.dataid=duf_filedatas.id) "                                                               */
/*                            " WHERE duf_exif.datetime IS NULL "                                                                                        */
/*                            " AND ( duf_filenames.name LIKE '%%.JPG' OR duf_filenames.name LIKE '%%.JPEG' ) " );                                       */
/*     r = sqlite3_get_table( pDb, sql, &presult, &row, &column, &errmsg );                                                                              */
/*     (* fprintf( stderr, "%ux%u : %s\n", row, column, sql ); *)                                                                                        */
/*     if ( r == SQLITE_OK )                                                                                                                             */
