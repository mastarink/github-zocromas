#include <stddef.h>
#include "duf_config_types.h"
#include "duf_ufilter_types.h"
#include "duf_levinfo_types.h"

#include "duf_config_ref.h"
#include "duf_option_types.h"
#include "duf_options_enum.h"
#include "duf_optable_def.h"

#include "duf_option_help.h"


extern duf_scan_callbacks_t duf_print_dir_callbacks /* __attribute( ( weak ) ) */ ;

/*
void duf_list_sccb( int ) __attribute__ ( ( weak ) );
void duf_list_targ( int *targc, char ***targv, long n ) __attribute__ ( ( weak ) );
void duf_add_targ( int *targc, char ***targv, const char *s ) __attribute__ ( ( weak ) );
void duf_clear_targ( int *targc, char ***targv, long n ) __attribute__ ( ( weak ) );
*/
const duf_longval_extended_t optable_interactive[] = {

  {.o = {DO_N( quit ) /*                */ , DO_A_N} /*                                    */ , DO_CL( CONTROL ) /*    */ ,
   /*      */ DO_O( NOFLAG, cli.act.v ), DO_FL( act, interactive ), DO_STAGE_SAME( 1 ) /*   */ , DO_H( get dir info ) /*                     */ },
  {.o = {DO_N( cd ) /*                  */ , DO_A_O} /*                                    */ , DO_CL( CONTROL ) /*    */ ,
   /*      */ DO_O( PDISTR, pdi ), DO_STAGE_SAME( 1 ) /*                                    */ , DO_H( pdi cd ) /*                           */ },
  {.o = {DO_N( pwd ) /*                 */ , DO_A_N} /*                                    */ , DO_CL( CONTROL ) /*    */ ,
   /*      */ DO_O( PDISTR, pdi ), DO_STAGE_SAME( 1 ) /*                                    */ , DO_H( pdi cd ) /*                           */ },

  {.o = {DO_N( lsfiles ) /*             */ , DO_A_O} /*                                    */ , DO_CL( CONTROL ) /*    */ ,
   /*      */ DO_O( PDISCCB, pdi ), DO_STAGE_SAME( 1 ),.call = {.fdesc = {.hi = {.sccb = &duf_print_dir_callbacks}}}, DO_H( pdi cd ) /*      */ },

  {.o = {DO_Q( "list-sccb" ) /*         */ , DO_A_O}, DO_T( VI_CALL ), DO_VI_CALL( list_sccb, 0 ), DO_STAGE_SAME( 1 ) /*                     */ },
  {.o = {DO_Q( "list-targ" ) /*         */ , DO_A_O}, DO_T( TN_CALL ), DO_TN_CALL( list_targ ), DO_STAGE_SAME( 1 ) /*                        */ },
  {.o = {DO_Q( "add-targ" ) /*          */ , DO_A_O}, DO_TF( TS_CALL, add_targ ), DO_STAGE_SAME( 1 ) /*                                      */ },
  {.o = {DO_Q( "clear-targ" ) /*        */ , DO_A_O}, DO_T( TN_CALL ), DO_TN_CALL( clear_targ ), DO_STAGE_SAME( 1 ) /*                       */ },


  {.o = {.name = NULL}}
};



/* vi: ft=c colorcolumn=3,44,59,60,95,96,123,145,146
*/
