...
/* 20150819.132024 */
static int
duf_scan_db_items2_row( duf_sqlite_stmt_t * pstmt_selector, duf_str_cb2_t str_cb2, duf_sccb_handle_t * sccbh, duf_node_type_t node_type )
{
  DEBUG_STARTR( r );
...
/*20150819.132038
 * 1. bind ufilter
 * 2. duf_scan_db_items2_row for each row from db by ufilter
 * */
static int
duf_scan_db_items2_sql( const char *csql_selector, duf_str_cb2_t str_cb2, duf_sccb_handle_t * sccbh, duf_node_type_t node_type )
{
  DEBUG_STARTR( r );
...
static int
duf_scan_db_items2_sql_set( const duf_sql_set_t * sql_set, duf_str_cb2_t str_cb2, duf_sccb_handle_t * sccbh, duf_node_type_t node_type )
{
  DEBUG_STARTR( r );
...
/* calling duf_sel_cb_(node|leaf) for each record by sql */
  ...
  {
    DOR( r, duf_scan_db_items2_sql_set( sql_set, str_cb2, sccbh, node_type ) );
    if ( r > 0 )
