                          ...
                          duf_scan_callback_file_t str_cb, void *str_cb_udata, duf_dirinfo_t * pdi, duf_scan_callbacks_t * sccb )
{
  ...
  if ( sccb )
  {
    ...
    if ( precord->nrow == 0 )
    {
      ...
      if ( have )
      {
        *pvalue = __duf_sql_ull_by_name( sccb->fieldset, precord, &have, 0 );
        r = precord->nrow;
      }
    }
  }
...
/*                                                                                                                                    */
/*     precord = ( duf_record_t ** ) sel_cb_udata;                                                                                    */
/*     if ( precord )                                                                                                                 */
/*     {                                                                                                                              */
/*       fprintf( stderr, "OGO %s :: {%p} %llu\n", __func__, ( void * ) precord, __duf_sql_ull_by_name( "dirid_here", precord, 0 ) ); */
/*       if ( precord->nrow == 0 )                                                                                                    */
/*       {                                                                                                                            */
/*         *pprecord = precord;                                                                                                       */
/*         r = precord->nrow;                                                                                                         */
...
/*										*/ duf_dbgfunc( DBG_START, __func__, __LINE__ );
  ...
  {
    DUF_TRACE( sql, 0, "DB Initialize %s", "OK" );
...
/*										*/ duf_dbgfunc( DBG_START, __func__, __LINE__ );
  ...
  if ( r != SQLITE_OK )
    ...
    DUF_TRACE( sql, 0, "DB Close %s", "OK" );

  r = sqlite3_shutdown(  );
  if ( r != SQLITE_OK )
    fprintf( stderr, "SQL DB Shutdown ERROR\n" );
  else
    DUF_TRACE( sql, 0, "DB Shutdown %s", "OK" );
                 ...
                 duf_dirinfo_t * pdi, duf_scan_callbacks_t * sccb, const char *fmt, va_list args )
{
  ...
  DUF_TRACE( sql, 2, "[%s] r=%d;  %u rows", sql, r, row );
 ...
 * */
        ...
        {
          rcb = ( sel_cb ) ( &rrecord, cargs, sel_cb_udata, str_cb, str_cb_udata, pdi, sccb );
