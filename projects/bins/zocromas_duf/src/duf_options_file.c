    ...
    duf_option_adata_t * paod )
{
  ...
  MAST_TRACE( options, 20, "@@@@stream options; stage:%s; source:%s", duf_optstage_name( cli, istage ), duf_optsource_name( cli, source ) );
      ...
      {
        {
          ...
          {
            MAST_TRACE( optsource, 0, "@@@@@@@@@%s:%s(%s)", duf_optstage_name( cli, istage ), duf_optsource_name( cli, source ),
  ...
  QTR;
  MAST_TRACE( options, 22, "@@@@stream options done; stage:%s; source:%s (%d:%s)", duf_optstage_name( cli, istage ),
              duf_optsource_name( cli, source ), QERRIND, QERRNAME );
...
SR( OPTIONS, infile_options_at_filepath, duf_config_cli_t * cli, duf_option_stage_t istage, const char *filepath, duf_option_adata_t * paod )
{
  ...
  MAST_TRACE( options, 20, "@@@@filepath options; stage:%s; filepath:%s", duf_optstage_name( cli, istage ), filepath );
  ...
  MAST_TRACE( options, 22, "@@@@filepath options done; stage:%s; filepath:%s (%d:%s)", duf_optstage_name( cli, istage ), filepath, QERRIND,
              QERRNAME );
    ...
    int optional, duf_option_source_t source, duf_option_adata_t * paod )
{
  ...
  MAST_TRACE( options, 20, "@@@@dir_and_file options; stage:%s; cfgdir:%s; filename:%s; source:%s", duf_optstage_name( cli, istage ), cfgdir,
              filename, duf_optsource_name( cli, source ) );
  if ( cfgdir )
  ...
  MAST_TRACE( options, 22, "@@@@dir_and_file options done; stage:%s; cfgdir:%s; filename:%s; source:%s (%d:%s)", duf_optstage_name( cli, istage ),
    ...
    duf_option_adata_t * paod, duf_option_source_code_t sourcecode )
{
  ...
  MAST_TRACE( options, 20, "@@@@cfgfile options; stage:%s; filename:%s", duf_optstage_name( cli, istage ), filename );

  CR( infile_options_at_dir_and_file, cli, istage, duf_cli_options_config_dir( cli ), filename, 3, optional, source, paod );

  MAST_TRACE( options, 22, "@@@@cfgfile options done; stage:%s; filename:%s (%d:%s)", duf_optstage_name( cli, istage ), filename, QERRIND, QERRNAME );
    ...
    duf_option_adata_t * paod, duf_option_source_code_t sourcecode )
{
  ...
  MAST_TRACE( optsource, 0, "@  *source:%s", duf_optsourcecode_name( cli, sourcecode ) );
  MAST_TRACE( options, 20, "@@@@cfgfile options; stage:%s; bfilename:%s", duf_optstage_name( cli, istage ), bfilename );
  ...
  mas_free( filename );
  MAST_TRACE( options, 22, "@@@@cfgfile options done; stage:%s; bfilename:%s (%d:%s)", duf_optstage_name( cli, istage ), bfilename, QERRIND,
              QERRNAME );
    ...
    MAS_UNUSED, duf_cpchar_void_func_t cb_prompt_interactive MAS_UNUSED, duf_option_adata_t * paod, duf_option_source_code_t sourcecode MAS_UNUSED )
{
  ...
  MAST_TRACE( optsource, 0, "@   source:%s", duf_optsourcecode_name( cli, sourcecode ) );
  MAST_TRACE( options, 20, "@@@@incfg options; stage:%s", duf_optstage_name( cli, istage ) );
  ...
  MAST_TRACE( options, 22, "@@@@incfg options done; stage:%s (%d:%s)", duf_optstage_name( cli, istage ), QERRIND, QERRNAME );
    ...
    duf_cpchar_void_func_t cb_prompt_interactive MAS_UNUSED, duf_option_adata_t * paod, duf_option_source_code_t sourcecode MAS_UNUSED )
{
  ...
  MAST_TRACE( optsource, 0, "@   source:%s", duf_optsourcecode_name( cli, sourcecode ) );
  MAST_TRACE( options, 20, "@@@@incfg `last` options; stage:%s", duf_optstage_name( cli, istage ) );
  CR( incfgf_options, cli, istage, duf_cli_options_config_file_name( cli ), "last", 1 /* optional */ , paod, DUF_OPTION_SOURCE_CFGLAST );
  MAST_TRACE( options, 22, "@@@@incfg `last` options done; stage:%s (%d:%s)", duf_optstage_name( cli, istage ), QERRIND, QERRNAME );
    ...
    duf_cpchar_void_func_t cb_prompt_interactive MAS_UNUSED, duf_option_adata_t * paod, duf_option_source_code_t sourcecode MAS_UNUSED )
{
  ...
  MAST_TRACE( optsource, 0, "@@@  source:%s", duf_optsourcecode_name( cli, sourcecode ) );
  MAST_TRACE( options, 20, "@@@@incfg stg options; stage:%s", duf_optstage_name( cli, istage ) );
  {
    const char *sn;
  ...
  MAST_TRACE( options, 22, "@@@@incfg stg options done; stage:%s (%d:%s)", duf_optstage_name( cli, istage ), QERRIND, QERRNAME );
    ...
    duf_cpchar_void_func_t cb_prompt_interactive MAS_UNUSED, duf_option_adata_t * paod, duf_option_source_code_t sourcecode MAS_UNUSED )
{
  ...
  MAST_TRACE( optsource, 0, "@@@  %s source:%s", duf_optstage_name( cli, istage ), duf_optsourcecode_name( cli, sourcecode ) );
/* if ( istage == DUF_OPTION_STAGE_FIRST (* XXX ???? XXX *)  ) */
  ...
  MAST_TRACE( options, 22, "@@@@stdin options done; stage:%s (%d:%s)", duf_optstage_name( cli, istage ), QERRIND, QERRNAME );
    ...
    duf_cpchar_void_func_t cb_prompt_interactive MAS_UNUSED, duf_option_adata_t * paod, duf_option_source_code_t sourcecode MAS_UNUSED )
{
  ...
  MAST_TRACE( optsource, 0, "@   source:%s", duf_optsourcecode_name( cli, sourcecode ) );
  MAST_TRACE( options, 20, "@@@@indirect options; stage:%s", duf_optstage_name( cli, istage ) );
  MAST_TRACE( options, 250, ">> targc:%d cli.targ_offset:%d", duf_cli_options_get_targc( cli ), duf_cli_options_get_targ_offset( cli ) );
/* for ( int ia = 0; ia < DUF_CONFIGG( cli.targ_offset ); ia++ ) */
  ...
  /* cf = DUF_CONFIGG( cli.targ.argv[ia] ); */
    ...
    MAST_TRACE( temp, 2, "@%s>> targv[%d]='%s'", duf_optstage_name( cli, istage ), ia, cf );
    if ( cf && *cf == '@' )
    {
      ...
      MAST_TRACE( temp, 2, "%s>> (%d) done targv[%d]='%s'", duf_optstage_name( cli, istage ), QERRIND, ia, cf );
    }
  }
  MAST_TRACE( options, 22, "@@@@indirect options done; stage:%s (%d:%s)", duf_optstage_name( cli, istage ), QERRIND, QERRNAME );
