...
duf_parse_option( duf_option_code_t codeval, int longindex, const char *optargg, duf_option_stage_t istage )
{
  ...
  if ( longindex < 0 )
  {
    ...
    DUF_TEST_R( r );
    DUF_TRACE( options, 1, "@@found by codeval of option %d (%c) => [--%s] (%s)", codeval, codeval > ' '
  ...
  if ( r >= 0 )
  {
    if ( extended )
    {
      ...
      case ':':
        ...
        DUF_TRACE( options, 3, "* options r: %d", r );
        break;
      case '?':
      ...
      default:
        DUF_MAKE_ERROR( r, DUF_ERROR_OPTION );
        DUF_TRACE( options, 3, "* options r: %d; codeval:%d;", r, codeval );
        break;
      }
  }
  DUF_TRACE( options, 1, "* options r: %d; codeval:%d;", r, codeval );
  DEBUG_ENDR( r );
}
