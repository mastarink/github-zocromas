    ...
    const duf_longval_extended_vtable_t * xtable, unsigned noo, duf_option_stage_t istage, duf_option_source_t source )
{
  ...
  if ( extended )
  {
    ...
    MAST_TRACE( options, 55, "@clarified typed: --%s / [%s]; (%d:%s)", extended->o.name, duf_xoption_description_tmp( cli, -1, extended ), QERRIND,
                QERRNAME );
...
#endif
    MAST_TRACE( options, 55, "@clarified cli option: %s (%d:%s)", duf_xoption_description_tmp( cli, -1, extended ), QERRIND, QERRNAME );
  }
  else
  {
  ...
  MAST_TRACE( options, +170, "@[%s]; arg:%s; istage:%d; `noo`:%d", duf_xoption_description_tmp( cli, -1, extended ), optargg, istage, noo );
