...
# define muc_T2T(_typid) typeof((*((mas_fundecl_t *)NULL)).r._typid)
...
# define muc_ST( _layer_id, _pre, _typid, _funname, ... ) muc_DT( _layer_id, , _typid, _funname, __VA_ARGS__ ) \
			{ \
			  mas_fundecl_t fundecl_={.layer_id= MUC_LAYER_ ## _layer_id,.r={.ul=0}}; \
...
/* # define muc_QDOCR( _x )                        ( (QNOERR) ? ( (SET_ERRIND(_x))  ) : 0 ) */
# define muc_CR(            _funname, ... )		QDOCR( muc_F2N(muc_,_funname)( __VA_ARGS__ ) )
# define muc_CRV(           _fun    , ... )		QDOCR( (_fun)( __VA_ARGS__) )
