#ifndef MAS_MUC_OPTION_TYPED_DEFS_H
#  define MAS_MUC_OPTION_TYPED_DEFS_H

#include <unistd.h>
#include "muc_printn_defs.h"                                         /* MUC_PRINTF */

#define MUC_NUMOPT( _no, _typ, _dopls, _conv) \
      if (QNOERR) \
      { \
	if ( !_no ) \
	{ \
	  ...
	  { \
	    _typ __v; \
	    if (*optargg=='?') \
	    { MUC_PRINTF(0, "%s=%lld", extended->o.name, (unsigned long long)( *p )); } \
	    else \
	    { \
...
#define MUC_MINMAXOPT(_no, _typ,_conv) \
      if (QNOERR) \
      { \
	if ( !_no ) \
	{ \
...
#define MUC_MOPT(_no, _typ,_mix,_conv) \
      if (QNOERR) \
      { \
	if ( !_no ) \
	{ \
...
#define MUC_OUTPUTFILE_A(_cli, _no, _typ, _stream_dot, _defoptarg, _defout ) \
      if ( QNOERR ) \
      { \
	if( !_no ) \
	{ \
	  ...
	  { \
	    muc_CR( set_file_special, _cli, s, mm->_stream_dot v.flag.overwrite, &mm->_stream_dot file, &mm->_stream_dot out, _defout, extended->call.value.u ); \
...
#define MUC_OUTPUTFILE(_cli, _no, _typ, _stream_dot, _defout ) MUC_OUTPUTFILE_A(_cli, _no, _typ, _stream_dot, NULL, _defout )
