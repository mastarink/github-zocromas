...
mucs_parse( int argc _uUu_, const char *argv[]_uUu_, const mucs_option_table_list_t * tablist _uUu_ )
{
  ...
  osrc = mucs_source_list_add_source_x( plist, MUCS_SOURCE_ENV, 0, "MAS_TEST_ENV", ":", "=", NULL );
  osrc = mucs_source_list_add_source_x( plist, MUCS_SOURCE_STRING, 0, "first=45 another=37:second=49;third=23", ":", "=", NULL );
