...
test_2a( int _uUu_ argc, const char _uUu_ * argv[], int nseries, const char *series_suffix, int variant _uUu_ )
{
  ...
  mucs_option_table_list_t test_tablist = {
    .next = NULL,.count = ( sizeof( options ) / sizeof( options[0] ) ),.name = "table-2a",.coptions = options, /* */
  };
...

  {
    ...
    mucs_source_t *osrc = mucs_source_list_add_source_x( plist, MUCS_SOURCE_STRING, 0, string_args, ":\r\n", "=", NULL );
