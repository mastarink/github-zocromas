...
test_1mul( int argc _uUu_, const char _uUu_ * argv[], int nseries, const char *series_suffix, int variant _uUu_ )
{
  ...
  mucs_option_table_list_t test_tablist = {
    .next = NULL,.count = ( sizeof( options ) / sizeof( options[0] ) ),.name = "table-1mul",.coptions = options, /* */
  };
...
#define NUM_NOPTS 3
  ...
  }
  {
    ...
    mucs_source_t *osrc = mucs_source_list_add_source_x( plist, MUCS_SOURCE_MARGV, xxargc, xxargv, NULL, "=", NULL );
