...
/* Now do options processing, get argument */
  ...
  masexam_next_group(  );
  masexam_exam( __LINE__, sizeof( v_int0 ) == 4 && v_int0 == 5437, "OK", "Error", "%d ? %d", v_int0, 5437 );
  masexam_exam( __LINE__, sizeof( v_int1 ) == 4 && v_int1 == 0x12, "OK", "Error", "%d ? %d", v_int1, 0x12 );
  masexam_exam( __LINE__, sizeof( v_int2 ) == 4 && v_int2 == 012, "OK", "Error", "%d ? %d", v_int2, 012 );
  masexam_exam( __LINE__, sizeof( v_int3 ) == 4 && v_int3 == INT_MAX, "OK", "Error", "%d ? %d", v_int3, INT_MAX );
  masexam_exam( __LINE__, sizeof( v_int4 ) == 4 && v_int4 == INT_MIN, "OK", "Error", "%d ? %d", v_int4, INT_MIN );

  masexam_next_group(  );
  masexam_exam( __LINE__, sizeof( v_long0 ) == 8 && v_long0 == 1099511627775L, "OK", "Error", "%ld ? %ld", v_long0, 0xffffffffffL );
  masexam_exam( __LINE__, sizeof( v_long1 ) == 8 && v_long1 == 0xff, "OK", "Error", "%ld ? %ld", v_long1, 0xffL );
  masexam_exam( __LINE__, sizeof( v_long2 ) == 8 && v_long2 == LONG_MAX, "OK", "Error", "%ld ? %ld", v_long2, LONG_MAX );
  masexam_exam( __LINE__, sizeof( v_long2 ) == 8 && v_long2 == 0x7fffffffffffffffL, "OK", "Error", "%lx ? %lx", v_long2, 0x7fffffffffffffffL );
  masexam_exam( __LINE__, sizeof( v_long2 ) == 8 && v_long2 == 9223372036854775807L, "OK", "Error", "%ld ? %ld", v_long2, 9223372036854775807L );
  masexam_exam( __LINE__, sizeof( v_long3 ) == 8 && v_long3 == 9223372036854775807L, "OK", "Error", "%ld ? %ld", v_long3, 9223372036854775807L );
  masexam_exam( __LINE__, sizeof( v_long4 ) == 8 && v_long4 == -12L, "OK", "Error", "%ld ? %ld", v_long4, -12L );
  masexam_exam( __LINE__, sizeof( v_long5 ) == 8 && v_long5 == LONG_MIN, "OK", "Error", "%ld ? %ld", v_long5, LONG_MIN );

  masexam_next_group(  );
  masexam_exam( __LINE__, sizeof( v_llong0 ) == 8 && v_llong0 == 5437LL, "OK", "Error", "%lld ? %lld (%d)", v_llong0, 5437LL, sizeof( v_llong0 ) );
  masexam_exam( __LINE__, sizeof( v_llong1 ) == 8 && v_llong1 == 0x12LL, "OK", "Error", "%lld ? %lld (%d)", v_llong1, 0x12LL, sizeof( v_llong1 ) );
  masexam_exam( __LINE__, sizeof( v_llong2 ) == 8 && v_llong2 == 012LL, "OK", "Error", "%lld ? %lld (%d)", v_llong2, 012LL, sizeof( v_llong2 ) );
  masexam_exam( __LINE__, sizeof( v_llong3 ) == 8
                && v_llong3 == LLONG_MAX - 1, "OK", "Error", "%llx/%lld ? %lld (%d)", v_llong3, v_llong3, LLONG_MAX - 1, sizeof( v_llong3 ) );
  masexam_exam( __LINE__, sizeof( v_llong4 ) == 8
...
/*
   ...
   masexam_exam(__LINE__, 0 == strcmp( "something", argsno[1] ), "OK", "Error", "'%s' ? '%s'", "something", argsno[1] );
   masexam_exam(__LINE__, 0 == strcmp( "wow", argsno[2] ), "OK", "Error", "'%s' ? '%s'", "wow", argsno[2] );
   masexam_exam(__LINE__, 0 == strcmp( "abrakadabra", argsno[3] ), "OK", "Error", "'%s' ? '%s'", "abrakadabra", argsno[3] );
   * */
  masexam_next_group(  );
  masexam_exam( __LINE__, thestring && 0 == strcmp( thestring, "asta manyana..." ), "OK", "Error", "%s ? %s", thestring, "asta manyana..." );
  masexam_next_group(  );
  masexam_exam( __LINE__, bitwise1 == ( long ) 0xfffffffffffff8ffL, "OK", "Error", "%lx ? bitwise1:%lx", ( long ) 0xfffffffffffff8ffL, bitwise1 );
  masexam_exam( __LINE__, bitwise2 == ( long ) 0x10304L, "OK", "Error", "%lx ? bitwise2:%lx", ( long ) 0x10304L, bitwise2 );
  masexam_exam( __LINE__, bitwise3 == ( long ) 0x10004L, "OK", "Error", "%lx ? bitwise3:%lx", ( long ) 0x10004L, bitwise3 );
  masexam_next_group(  );
  masexam_exam( __LINE__, c == -1, "OK", "Error", "err (c):%d", c );
  masexam_exam( __LINE__, err == 0, "OK", "Error", "err:%d", err );
