...
int
test_5( int _uUu_ argc, const char _uUu_ * argv[], int nseries, const char *series_suffix, int variant _uUu_ )
{
  ...
  mucs_option_table_list_t test_tablist = {
    .next = NULL,.optcount = ( sizeof( options ) / sizeof( options[0] ) ) - 1,.name = "table-5",.coptions = options, /* */
  };
...
",
  ...
  }
  {
    ...
    mucs_source_t *osrc =
            mucs_source_list_add_source_x( plist, MUCS_SOURCE_STREAM, NULL /*name */ , 0, fcmds, 0 /* min_pass */ , 0 /* npasses */ , ":\r\n", "=",
                                           NULL );
