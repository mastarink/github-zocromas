...
test_4( int _uUu_ argc, const char _uUu_ * argv[], int nseries, const char *series_suffix, int variant _uUu_ )
{
  ...
  mucs_option_table_list_t test_tablist = {
    .next = NULL,.optcount = ( sizeof( options ) / sizeof( options[0] ) ) - 1,.name = "table-4",.coptions = options, /* */
  };
...

  {
    ...
    mucs_source_t *osrc =
            mucs_source_list_add_source_x( plist, MUCS_SOURCE_ENV, NULL /*name */ , 0, "MASTEST_4", 0 /* min_pass */ , 0 /* npasses */ , ":", "=",
                                           NULL );
    ...
    EXAMX( ge && 0 == mas_strcmp( ge, string_args ), "MASTEST_4: %s ? %s (%d)", ge, string_args, strlen( ge ) );
  ...
  /* EXAMX( !mucs_error_source( osrc ),  "mulconfnt_error: %d", mucs_error_source( osrc ) ); */
    ...
    EXAMX( bitwise3 == ( long ) 0x10004L, bitwise3 );
#endif
    unsetenv( "MASTEST_4" );
