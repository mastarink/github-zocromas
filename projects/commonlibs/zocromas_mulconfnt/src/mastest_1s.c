...
test_1s( int argc _uUu_, const char *argv[], int nseries, const char *series_suffix, int variant _uUu_ )
{
  ...
  mucs_option_t options[] = {
    ...
    , {.s = {"string1", 's',.restype= MUCS_RTYP_STRING, .p=&v_string1,.flags = 0 | MUCS_FLAG_AUTOFREE}}
    , {.s = {"num0", 'n',.restype= MUCS_RTYP_INT, .p=&v_int0}}
    , {.s = {"num1", 'x',.restype= MUCS_RTYP_INT, .p=&v_int1}}
    , {.s = {"num2", 0,.restype= MUCS_RTYP_INT, .p=&v_int2}}
    , {.s = {"num3", 'u',.restype= MUCS_RTYP_INT, .p=&v_int3}}
  ...
  mucs_option_table_list_t test_tablist = {
    .next = NULL,.optcount = ( sizeof( options ) / sizeof( options[0] ) ) - 1,.name = "table-1s",.coptions = options, /* */
  };
