...
int
mas_opts_restore_nosection( void *popts, const char *s, mas_option_parse_t * opt_table, size_t opt_table_size )
{
  int r = -1;
  ...
  for ( int io = 0; io < opt_table_size; io++ )
  {
    ...
    if ( 0 == strncmp( s, opt_table[io].name, strlen( opt_table[io].name ) ) )
    {
      const char *se;

      r = 0;
      /* HMSG( "%d. %s (%lu:%lu) : %s", io, opt_table[io].name, ( unsigned long ) opt_table[io].type, ( unsigned long ) opt_table[io].offset, */
      /*       s );                                                                                                                           */
...
int
_mas_opts_restore_path( const char *fpath, void *popts, mas_option_parse_t * opt_table, size_t opt_table_size,
    ...
    if ( f )
    {
      ...
      while ( !feof( f ) )
      {
        ...
        if ( section && at_section_func )
        {
          ...
          /* IEVAL( r, mas_opts_restore_at_section( section, s ) ); */
        }
        else
        {
          r = mas_opts_restore_nosection( popts, s, opt_table, opt_table_size );
          if ( r < 0 )
