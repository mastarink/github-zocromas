...
/**
https://raw.githubusercontent.com/MariaDB/mariadb-connector-c/master/unittest/libmariadb/async.c
**/
...
static int
async1( MYSQL * unused __attribute__ ( ( unused ) ) )
{
  int err = 0, rc;
  MYSQL mysql, *ret;
  MYSQL_RES *res;
  ...
  /* Returns 0 when done, else flag for what to wait for when need to block. */
    status = mysql_real_connect_start( &ret, &mysql, hostname, username, password, NULL, 0, NULL, 0 );
    while ( status )
    {
...
static int
test_conc131( MYSQL * unused __attribute__ ( ( unused ) ) )
{
  int rc;
...
static int
test_conc129( MYSQL * unused __attribute__ ( ( unused ) ) )
{
  MYSQL *mysql;
