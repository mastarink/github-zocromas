...
static int __attribute__ ( ( unused ) ) wait_for_mysql( MYSQL * mysql, int status )
{
  int r = 0;
  struct pollfd pfd;
  int timeout, res;
...
run_query( const char *host, const char *user, const char *password )
{
...
#else
  while ( ( status = mysql_real_connect_cont( &ret, &mysql, status ) ) )
  {
    fprintf( stderr, "%d status: %x\n", __LINE__, status );
    status = wait_for_mysql( &mysql, status );
    fprintf( stderr, "%d status: %x\n", __LINE__, status );
  ...
  fprintf( stderr, "%d status: %x\n", __LINE__, status );
  while ( ( status = mysql_real_query_cont( &err, &mysql, status ) ) )
  {
    status = wait_for_mysql( &mysql, status );
    fprintf( stderr, "%d status: %x\n", __LINE__, status );
  }
