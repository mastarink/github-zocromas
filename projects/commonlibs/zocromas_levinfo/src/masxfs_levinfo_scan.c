                                               ...
                                               masxfs_entry_callback_t * cbs, void *data, masxfs_levinfo_flags_t flags, masxfs_depth_t reldepth )
{
  ...
  if ( liparent && lithis && cbs )
  {
    ...
    {
      if ( ( flags & MASXFS_CB_FROM_ROOT ) && reldepth <= 1 )        /* Once! ; ==1 for dir ; ==0 for file at initial path */
      {
        ...
        for ( masxfs_levinfo_t * li = lia; rGOOD && li < lithis; li++ )
        {
          ...
          if ( li->lidepth == ( masxfs_depth_t ) ( li - lia ) )
          {
            ...
            rC( masxfs_levinfo_scan_entry_single_internal_1cb( li, typeflags, cb, data, flags & ~MASXFS_CB_FROM_ROOT, li - lithis ) );
          }
          else
          {
      ...
      rC( masxfs_levinfo_scan_entry_single_internal_1cb( lithis, typeflags, cb, data, flags & ~MASXFS_CB_FROM_ROOT, reldepth ) );
