...
mas_vlog( const char *func, int line, int merrno, const char *fmt, va_list args )
{
  ...
  li->lserial = li->plcontrol ? li->plcontrol->serial : 0;
  li->lstatus = li->plcontrol ? li->plcontrol->status : 0;
  li->rserial = li->prcontrol ? li->prcontrol->serial : 0;
  li->rstatus = li->prcontrol ? li->prcontrol->status : 0;
