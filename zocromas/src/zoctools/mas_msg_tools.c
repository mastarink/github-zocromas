    ...
    if ( ctrl.is_server )
    {
      ...
      if ( use_curses )
      {
        ...
        case MAS_THREAD_LISTENER:
          MFP( "L%lu:%u ", plcontrol ? plcontrol->serial : 0xffffffff, plcontrol ? plcontrol->status : 999 );
          break;
        case MAS_THREAD_TRANSACTION:
          MFP( "R%lu:%u @ L%lu:%u ", prcontrol ? prcontrol->serial : 0xffffffff, prcontrol ? prcontrol->status : 999,
               prcontrol->plcontrol ? prcontrol->plcontrol->serial : 0xffffffff,
               prcontrol->plcontrol ? prcontrol->plcontrol->status : 999 );
          break;
      ...
      else
      {
        ...
        case MAS_THREAD_LISTENER:
          MFP( "pth:\x1b[1;%dm(%s) L%lu:%u\x1b[0m:", 34, mas_thread_type_name( thtype ),
               plcontrol ? plcontrol->serial : 0xffffffff, plcontrol ? plcontrol->status : 999 );
          break;
        case MAS_THREAD_TRANSACTION:
          MFP( "pth:\x1b[1;%dm(%s) R%lu:%u @ L%lu:%u\x1b[0m:", 35, mas_thread_type_name( thtype ),
               prcontrol ? prcontrol->serial : 0xffffffff, prcontrol ? prcontrol->status : 999,
               prcontrol->plcontrol ? prcontrol->plcontrol->serial : 0xffffffff,
               prcontrol->plcontrol ? prcontrol->plcontrol->status : 999 );
          break;
        default:
