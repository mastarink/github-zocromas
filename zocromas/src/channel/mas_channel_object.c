...
static int
__mas_channel_init( mas_channel_t * pchannel, chn_type_t type, const char *host, size_t hostlen, int port )
{
  int r;
...
static int
_mas_channel_init( mas_channel_t * pchannel, chn_type_t type, const char *host, size_t hostlen, int port )
{
  int r = 0;
  ...
  if ( mas_channel_test( pchannel ) && !pchannel->opened )
    r = __mas_channel_init( pchannel, type, host, hostlen, port );
  return r;
}

int
mas_channel_init( mas_channel_t * pchannel, chn_type_t type, const char *host, size_t hostlen, int port )
{
  int r = 0;

  if ( host )
    r = _mas_channel_init( pchannel, type, host, hostlen, port );
  if ( r < 0 )
  {
    P_ERR;
  }
